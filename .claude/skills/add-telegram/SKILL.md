...
The chat ID, name, and folder name are needed. Use `python -m src.main setup-step register` with the appropriate flags.

For a main chat (responds to all messages):

```bash
python -m src.main setup-step register --jid "tg:<chat-id>" --name "<chat-name>" --folder "telegram_main" --trigger "@${ASSISTANT_NAME}" --channel telegram --no-trigger-required --is-main
```

For additional chats (trigger-only):

```bash
python -m src.main setup-step register --jid "tg:<chat-id>" --name "<chat-name>" --folder "telegram_<group-name>" --trigger "@${ASSISTANT_NAME}" --channel telegram
```
