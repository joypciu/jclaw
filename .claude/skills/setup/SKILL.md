...
Run setup steps automatically. Only pause when user action is required (channel authentication, configuration choices). Setup uses `bash setup.sh` for bootstrap, then `python -m src.main setup-step <name>` for all other steps. Steps emit JSON status to stdout. Verbose logs go to `logs/setup.log`.
...
Run `python -m src.main setup-step environment` and parse the JSON status.
...
```bash
grep -q "CONTAINER_RUNTIME_BIN = 'container'" src/container_runtime.py && echo "ALREADY_CONVERTED" || echo "NEEDS_CONVERSION"
```
...
Run `python -m src.main setup-step container --runtime <chosen>` and parse the JSON status.
...
**No:** `python -m src.main setup-step mounts --empty`
**Yes:** Collect paths/permissions. `python -m src.main setup-step mounts --json '{"allowedRoots":[...],"blockedPatterns":[],"nonMainReadOnly":true}'`
...
Run `python -m src.main setup-step service` and parse the JSON status.
...
Run `python -m src.main setup-step verify` and parse the JSON status.
...
- MOUNT_ALLOWLIST=missing → `python -m src.main setup-step mounts --empty`
...
**No response to messages:** Check trigger pattern. Main channel doesn't need prefix. Check DB: `python -m src.main setup-step verify`. Check `logs/nanoclaw.log`.
